...
from gui_app import hf_downloader
...
class TestApiLayer(unittest.TestCase):
    ...
    def test_get_json_retry_then_success(self):
        with mock.patch("urllib.request.urlopen",
             ...
             mock.patch("gui_app.hf_downloader.time.sleep"):
            data = hf_downloader.get_json("http://x/api", params={"a": "b"})
    ...
    def test_get_json_gives_up_after_retries(self):
        with mock.patch("urllib.request.urlopen",
             ...
             mock.patch("gui_app.hf_downloader.time.sleep"):
            with self.assertRaises(hf_downloader.HttpError):
                hf_downloader.get_json("http://x/api")
    ...
    def test_search_models_params(self):
        with mock.patch("urllib.request.urlopen", return_value=_json_resp([])), \
            ...
            hf_downloader.api_search_models("gguf", sort="downloads", limit=10)
    ...
    def test_search_models_cursor(self):
        ...
        with mock.patch("urllib.request.urlopen",
             ...
             mock.patch("urllib.request.Request", wraps=urllib.request.Request) as req:
            data, cur = hf_downloader.api_search_models("gguf", cursor="xyz=")
    ...
    def test_search_models_no_cursor_when_last_page(self):
        with mock.patch("urllib.request.urlopen",
            ...
            data, cur = hf_downloader.api_search_models("gguf")
    ...
    def test_list_files_recursive(self):
        with mock.patch("urllib.request.urlopen", return_value=_json_resp([])), \
             mock.patch("urllib.request.Request", wraps=urllib.request.Request) as req:
            hf_downloader.api_list_files("unsloth/Qwen-GGUF")
        url = req.call_args.args[0]
    ...
    def test_resolve_download_url(self):
        url = hf_downloader.resolve_download_url("unsloth/Qwen-GGUF",
                                                 "Qwen-GGUF-Q4_K_M.gguf")
        self.assertEqual(url, hf_downloader.HF_ENDPOINT
...
class TestClassifyFiles(unittest.TestCase):
    ...
    def test_classify(self):
        ...
        r = hf_downloader.classify_files(files)
    ...
    def test_mmproj_any_case(self):
        r = hf_downloader.classify_files([{"type": "file", "path": "MMPROJ-X.gguf"}])
...
class TestDownloadFile(unittest.TestCase):
    ...
    def _run(self, urlopen_side_effect, prefill=0, cancel=None, expected_size=0):
        ...
        with mock.patch("urllib.request.urlopen",
             ...
             mock.patch("gui_app.hf_downloader.time.sleep"):
            hf_downloader._download_file("http://x/model.gguf", str(self.dest),
    ...
    def test_cancel_midway_deletes_partial(self):
        ...
        with mock.patch("urllib.request.urlopen", return_value=r), \
             mock.patch("urllib.request.Request", wraps=urllib.request.Request), \
             mock.patch("gui_app.hf_downloader.time.sleep"):
            with self.assertRaises(hf_downloader.DownloadCancelled):
                hf_downloader._download_file("http://x/model.gguf", str(self.dest),
                                             "model.gguf", self.logs.append,
...
class TestChunkedDownload(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for name, val in [("CHUNK_MIN_SIZE", 1024), ("CHUNK_SEG_SIZE", 512),
                          ("CHUNK_WORKERS", 3)]:
            p = mock.patch.object(hf_downloader, name, val)
            p.start()
    ...
    def _chunked(self, side_effect, cancel=None):
        with mock.patch("urllib.request.urlopen", side_effect=side_effect), \
             mock.patch("gui_app.hf_downloader.time.sleep"):
            hf_downloader._download_file_chunked(
    ...
    def test_range_ignored_raises_fallback(self):
        ...
        with self.assertRaises(hf_downloader.ChunkedFallback):
            self._chunked(_open)
        self.assertEqual(self._parts(), [])
    ...
    def test_download_files_range_ignored_falls_back(self):
        ...
        with mock.patch("urllib.request.urlopen", side_effect=_open), \
             mock.patch("gui_app.hf_downloader.time.sleep"), \
             mock.patch("gui_app.hf_downloader._fetch_sizes",
                        return_value={"model.gguf": self.SIZE}):
            r = hf_downloader.download_files(
                "unsloth/Qwen-GGUF", ["model.gguf"], str(self.dest.parent),
    ...
    def test_failure_degrades_with_prefix_merged(self):
        ...
        with mock.patch("urllib.request.urlopen", side_effect=OSError("reset")), \
             mock.patch("gui_app.hf_downloader.time.sleep"), \
             mock.patch.object(hf_downloader, "_download_file") as df:
            hf_downloader._download_file_chunked(
                "http://x/model.gguf", str(self.dest), "model.gguf",
    ...
    def test_cancel_during_urlopen_returns_quickly(self):
        ...
        复现改动A 解决的场景：watcher 只覆盖 read 阶段，urlopen 建连/等响应头
        阻塞时只能靠子线程轮询 cancel_event 才能快速响应。
        """
        ...
        self.assertIsInstance(result_box.get("e"), hf_downloader.DownloadCancelled)
        blocker.set()
...
class TestDownloadFiles(unittest.TestCase):
    """download_files：两级目录组织 / 失败继续（保留断点）/ 取消中断。"""
    ...
    def _run(self, urlopen_side_effect, files=("model.gguf", "mmproj-x.gguf"),
        ...
        with mock.patch("urllib.request.urlopen",
             ...
             mock.patch("gui_app.hf_downloader.time.sleep"), \
             mock.patch("gui_app.hf_downloader._fetch_sizes", return_value=sizes):
            return hf_downloader.download_files(
                "unsloth/Qwen-GGUF", list(files), str(self.root),
    ...
    def test_two_level_layout_and_progress_order(self):
        ...
        with mock.patch("urllib.request.urlopen", side_effect=[
            ...
            r = hf_downloader.download_files(
                "unsloth/Qwen-GGUF", ["model.gguf", "mmproj-x.gguf"],
...
class TestConcurrencyGuard(unittest.TestCase):
    ...
    def test_begin_twice_rejected(self):
        self.assertTrue(hf_downloader.begin_download())
        try:
            self.assertFalse(hf_downloader.begin_download())  # 占坑中 → 拒绝
        finally:
            hf_downloader.end_download()
        self.assertTrue(hf_downloader.begin_download())  # 释放后可再次开始
        hf_downloader.end_download()
...
class TestModelDownloadMixin(unittest.TestCase):
    ...
    def test_open_repo(self):
        with mock.patch("webbrowser.open") as mo:
            r = self.api.hf_open_repo("unsloth/Qwen-GGUF")
        self.assertTrue(r["ok"])
        self.assertEqual(r["url"], hf_downloader.HF_ENDPOINT + "/unsloth/Qwen-GGUF")
        mo.assert_called_once_with(r["url"])

    def test_open_repo_rejects_bad_id(self):
        with mock.patch("webbrowser.open") as mo:
            r = self.api.hf_open_repo("../etc/passwd")
        self.assertFalse(r["ok"])
    ...
    def test_repo_files_ok(self):
        ...
        with mock.patch("gui_app.hf_downloader.api_list_files", return_value=files) as m, \
             mock.patch("gui_app.llama_cpp.get_models_dir",
                        return_value=Path("M:/models")):
            r = self.api.hf_repo_files("unsloth/Qwen-GGUF")
        self.assertTrue(r["ok"])
        ...
        m.assert_called_once_with("unsloth/Qwen-GGUF")

    def test_download_models_pushes_progress(self):
        events = []

        def _fake_download(repo_id, files, dest_root, log_fn=None,
                           progress_cb=None, cancel_event=None, revision="main"):
            log_fn("开始下载")
        ...
        with mock.patch("gui_app.hf_downloader.begin_download", return_value=True) as begin, \
             mock.patch("gui_app.hf_downloader.download_files", side_effect=_fake_download), \
             mock.patch("gui_app.hf_downloader.end_download") as end, \
             mock.patch("gui_app.llama_cpp.get_models_dir",
                        return_value=Path("M:/models")), \
             mock.patch("gui_app.api_mixins.models.js_pusher") as pusher:
            r = self.api.hf_download_models("u/q", ["model.gguf"])
    ...
    def test_download_rejected_when_busy(self):
        with mock.patch("gui_app.hf_downloader.begin_download", return_value=False), \
             mock.patch("gui_app.hf_downloader.download_files") as dl:
            r = self.api.hf_download_models("u/q", ["model.gguf"])
        self.assertFalse(r["ok"])
    ...
    def test_download_empty_files_rejected(self):
        with mock.patch("gui_app.hf_downloader.begin_download") as begin:
            r = self.api.hf_download_models("u/q", [])
        self.assertFalse(r["ok"])
        begin.assert_not_called()

    def test_cancel(self):
        with mock.patch("gui_app.hf_downloader.set_cancel") as sc:
            r = self.api.hf_cancel_download()
        self.assertTrue(r["ok"])
    ...
    def test_download_exception_still_pushes_done(self):
        ...
        with mock.patch("gui_app.hf_downloader.begin_download", return_value=True), \
             mock.patch("gui_app.hf_downloader.download_files",
                        side_effect=RuntimeError("boom")), \
             mock.patch("gui_app.hf_downloader.end_download"), \
             mock.patch("gui_app.llama_cpp.get_models_dir",
                        return_value=Path("M:/models")), \
             mock.patch("gui_app.api_mixins.models.js_pusher") as pusher:
            r = self.api.hf_download_models("u/q", ["model.gguf"])
