...
Hammy 是仓鼠党整理入库杂乱视频的小工具：从视频中抽取关键帧，发送给支持视觉（Vision）的
OpenAI 兼容 API（本地或云端均可），由 AI 自动生成标题、剧情描述与标签，再据此
重命名视频文件，并可选生成 Kodi/Jellyfin/Emby 可用的 `.nfo` 媒体信息文件。
...
- **提示词**：结构层（只读 JSON 约束）与内容层（可编辑）分离；支持自定义预设与「优先标签」。
...
- Qwen3.X系列：4B即可正确按格式输出，单图物体和角色的识别能力显著强于Gemma4，但多图理解不如gemma4且生成描述生硬，需要修改提示词引导；
