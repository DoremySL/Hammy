...
# ── 优先标签（全局，独立存储于 _workspace/priority_tags.json） ──


def normalize_priority_items(items: Any) -> List[Dict[str, str]]:
    """规范化优先标签列表：剔除非 dict 与空关键词，keyword/description 去首尾空格。"""
...
def save_priority_tags(enabled: Any, items: Any) -> Dict[str, Any]:
    """保存优先标签到 _workspace/priority_tags.json。"""
    data = {
...
def build_priority_tags_section(enabled: Any, items: Any) -> str:
    """拼出注入提示词的「优先标签」段落；未启用或无有效标签时返回空串。"""
    if not enabled:
        return ""
